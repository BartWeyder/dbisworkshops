...
alter table answer 
    add constraint ANSWER_TITILE_CHECK check(REGEXP_LIKE(ANSWERTITLE, '^[?-??-???????????A-z -]{0,526}$'));
...
alter table role  
    add constraint ROLE_NAME_CHECK check(REGEXP_LIKE(ROLENAME, '^[A-z]{1,20}$'));

alter table TAG  
    add constraint TITLE_CHECK check(REGEXP_LIKE(TITLE, '^[A-z]{1,20}$'));
    
alter table "USER"
    add constraint PHONE_CHECK check(REGEXP_LIKE(PHONE, '^(\+[0-9]{1,3}|0)[0-9]{3}( ){0,1}[0-9]{7,8}$'));

alter table "USER"
    add constraint NAME_CHECK check(REGEXP_LIKE(NAME, '^[?-??-???????????A-z -]{5,100}$'));
